...
public class EasyQuizActivity extends AppCompatActivity {
    ...
    private final int TOTAL_NUM_QUESTIONS = EasyQuestionBankController.getQuestions().length;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mCheatButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Question currentQuestion =  EasyQuestionBankController.getQuestions()[mCurrentIndex];
                char correctAnswer = EasyQuestionBankController.getQuestions()[mCurrentIndex].getCorrectAnswer();
                Intent intent = CheatActivity.newIntent(EasyQuizActivity.this, correctAnswer, currentQuestion);
        ...
        mResultSummaryButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = ResultSummaryActivity.newIntent(EasyQuizActivity.this, TOTAL_NUM_QUESTIONS, mAnsweredCounter, mScoreResult, mCheatCounter);
    ...
    private void updateQuestion() {
        int question = EasyQuestionBankController.getQuestions()[mCurrentIndex].getTextResId();
    ...
    private void updateAnswerButtonsVisibility() {
        if (EasyQuestionBankController.getQuestions()[mCurrentIndex].getHasAnswered()) {
    ...
    private void updateCheatSystemVisibility() {
        ...
        if (mCheatCounter == TOTAL_CHEAT_ATTEMPT || mIsCheaterForThisQuestion || EasyQuestionBankController.getQuestions()[mCurrentIndex].getHasAnswered()) {
    ...
    private void checkAnswer(char userPressedTrue) {
        ...
        EasyQuestionBankController.getQuestions()[mCurrentIndex].setHasAnswered(true);
        updateAnswerButtonsVisibility();
        updateCheatSystemVisibility();

        // displays the result of the question in TOAST
    ...
    private void navigateNextQuestion() {
        mCurrentIndex = (mCurrentIndex + 1) % EasyQuestionBankController.getQuestions().length;
        navigateQuestions();
    }
    ...
    private void reset() {
        ...
        EasyQuestionBankController.resetHasAnswered();
        updateQuestionHeader();
        shuffleQuestions();
        updateQuestion();
        updateAnswerButtonsVisibility();
    ...
    private void shuffleQuestions() {
        ...
        if (mAnsweredCounter == 0 && !mHasShuffled && mCheatCounter == 0) {
            EasyQuestionBankController.shuffleQuestions();
            mHasShuffled = true;
        }
    }
